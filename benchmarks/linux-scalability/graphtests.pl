...
my $graphtitle = "linux scalability - average runtimes";
...
foreach $name (@namelist) {
    ...
    for (my $i = 1; $i <= 8; $i++) {
	open F, "$dir/Results/$name/linux-scalability-$i";
	my $total = 0;
	my $count = 0;
	my $min = 1e30;
	my $max = -1e30;
	...
	if ($count > 0) {
	    ...
	    print "oops count is zero, $name, linux-scalability-$i\n";
	}
...
print PLOT "set output \"$dir/linux-scalability.pdf\"\n";
print PLOT "set title \"$graphtitle\"\n";
print PLOT "set ylabel \"Average Runtime (per thread, seconds)\"\n";
